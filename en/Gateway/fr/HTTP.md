...
HTTP est sans état, ce qui signifie que chaque requête d'un client à un serveur est traitée de manière indépendante, sans mémoire des interactions précédentes à moins qu'elles ne soient explicitement gérées à l'aide de mécanismes tels que les cookies ou le stockage de session.
